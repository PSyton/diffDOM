(function () {
  ...
  diffDOM.prototype = {
      ...
      attr1.forEach(function (attr) {
        ...
        if (pos === -1) {
          ...
          if (t1.attributes[attr] !== t2.attributes[attr]) {
            diffs.push(new Diff({
              action: Actions.modifyAttribute,
              route: route,
              name: attr,
      ...
      for (i = 0; i < last; i += 1) {
        ...
        if (childNodesLengthDifference) {
          ...
          if (e1 && !e2) {
            if (e1.nodeName === '#text') {
              diffs.push(new Diff({
                action: Actions.removeTextElement,
    ...
    applyVirtualDiff: function (tree, diff) {
      ...
      switch (diff.action) {
        ...
        case Actions.removeAttribute:
          ...
          break;
        case Actions.modifyTextElement:
          ...
          break;
        case Actions.modifyValue:
          node.value = diff.newValue;
          break;
        case Actions.modifyComment:
          node.data = diff.newValue;
          break;
        case Actions.modifyChecked:
          node.checked = diff.newValue;
          break;
        case Actions.modifySelected:
          node.selected = diff.newValue;
          break;
        case Actions.replaceElement:
        ...
        case Actions.addElement:
          ...
          break;
        case Actions.removeTextElement:
          parentNode.childNodes.splice(nodeIndex, 1);
          if (parentNode.nodeName === 'TEXTAREA') {
            delete parentNode.value;
          }
          break;
        case Actions.addTextElement:
          route = diff.route.slice();
          c = route.splice(route.length - 1, 1)[0];
          newNode = {};
    ...
    undoDiff: function (tree, diff) {
      ...
      switch (diff.action) {
        case Actions.addAttribute:
          diff.action = Actions.removeAttribute;
        ...
        case Actions.modifyAttribute:
          ...
          break;
        case Actions.removeAttribute:
          diff.action = Actions.addAttribute;
          this.applyDiff(tree, diff);
          break;
        case Actions.modifyTextElement:
          swap(diff, 'oldValue', 'newValue');
        ...
        case Actions.relocateGroup:
          ...
          break;
        case Actions.removeElement:
          diff.action = Actions.addElement;
          this.applyDiff(tree, diff);
          break;
        case Actions.addElement:
          diff.action = Actions.removeElement;
          this.applyDiff(tree, diff);
          break;
        case Actions.removeTextElement:
          diff.action = Actions.addTextElement;
          this.applyDiff(tree, diff);
          break;
        case Actions.addTextElement:
          diff.action = Actions.removeTextElement;
          this.applyDiff(tree, diff);
          break;
        default:
          console.log('unknown action');
